<template>
   <div class="historyChartsWrap" onselectstart="return false">
      ...
      <ThirdChart :dataSet="dataSet.hum"> </ThirdChart>
          ...
          makeDataSet(newValue){
            if (newValue.length>0){
              let temp = newValue.filter((item, index, arr) => {
                ...
                if (index == 0 || index == arr.length-1) return true;//включаем первый элемент всегда
              ...
              let splitter = 1;
              if (this.selector.day) splitter = 4;
              if (this.selector.week) splitter = 20;
              if (this.selector.month) splitter = 50;
              if (this.selector.year) splitter = 70;
              if (this.selector.interval) splitter = (-(-((new Date(this.endTime)-new Date(this.startTime))/(24*3600*1000)).toFixed())+1)*2;
